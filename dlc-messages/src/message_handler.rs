...
/// receiving. It is necessary to call the [`MessageHandler::connection_dropped`]
/// method when the connection to a peer is dropped for the segmentation protocol
/// to work as intended.
...
impl MessageHandler {
    ...
    /// Tell the message handler that the connection to the peer with given public
    /// key was dropped (necessary for good functioning of the segmentation protocol).
    pub fn connection_dropped(&mut self, pubkey: &PublicKey) {
        self.segment_readers.lock().unwrap().remove(pubkey);
    }
...
impl CustomMessageHandler for MessageHandler {
    fn handle_custom_message(
        ...
        match msg {
            ...
            _ => unreachable!("Segmentation chunk should have been processed above"),
        };
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn resets_segment_reader_on_connection_drop_test() {
        let input = include_str!("./test_inputs/segment_start_msg.json");
        let msg: WireMessage = WireMessage::SegmentStart(serde_json::from_str(&input).unwrap());
        let mut handler = MessageHandler::new();
        let node_id = some_pk();
        handler
            .handle_custom_message(msg.clone(), &node_id)
            .expect("to be able to handle the message");
        handler.connection_dropped(&node_id);
        handler
            .handle_custom_message(msg, &node_id)
            .expect("to be able to handle the message after dropping connection");
    }
