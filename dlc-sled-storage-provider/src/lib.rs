...
impl SledStorageProvider {
    ...
    fn channel_tree(&self) -> Result<Tree, Error> {
        self.db
            .open_tree(&[CHANNEL_TREE])
            .map_err(|e| Error::StorageError(format!("Error opening channel tree: {}", e)))
    }
}
...
impl Storage for SledStorageProvider {
    ...
    fn get_confirmed_contracts(&self) -> Result<Vec<SignedContract>, Error> {
        println!("Prefix: {:?}", u8::from(ContractPrefix::Confirmed));
        self.get_data_with_prefix(
            &self.contract_tree()?,
    ...
    fn upsert_channel(&mut self, channel: &Channel) -> Result<(), Error> {
        self.channel_tree()?
            .transaction(|db| {
                let serialized = match serialize_channel(channel) {
                    Ok(b) => b,
                    Err(e) => sled::transaction::abort(e)?,
                };
                match channel {
                ...
                db.insert(&channel.get_id(), serialized)?;
                Ok(())
            })
            .map_err(to_storage_error)?;
        Ok(())
